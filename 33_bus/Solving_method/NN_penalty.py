...
def compute_objective_single(l_sq, pv_p, p_available):
    ...
    return loss_curtailment + loss_network, loss_network, curtailment
...
network_loss_list = []     # 网损量
curtailment_list = []      # 弃光量
...
for i in range(n_test):
    ...
    with torch.no_grad():
        ...
        # 反标准化
        # y_pred_raw = y_pred_norm * Y_scale_tensor + Y_mean_tensor
        y_pred_np = y_pred_norm.cpu().numpy().flatten()
    ...
    # 计算目标函数
    obj_value, network_loss, curtailment = compute_objective_single(
        l_sq, pv_p_nn, p_available
    )
    ...
    network_loss_list.append(network_loss)
...
network_loss_values = np.array(network_loss_list)
curtailment_values = np.array(curtailment_list)
...
np.savez("Test_result/results_P_NN.npz", **results)
print(f"\n结果已保存到: Test_result/results_P_NN.npz")
...
for j in range(n_test):
    ...
    label_obj_value, _, _ = compute_objective_single(
        l_sq_j, pv_p_label[j], p_avail_np[j]
    )
    label_objective_values_list.append(label_obj_value)
...
print(f"  NN 平均网损量:      {np.mean(network_loss_values):.6f}")
print(f"  NN 平均弃光量:      {np.mean(curtailment_values):.6f}")
