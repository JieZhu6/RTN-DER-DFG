...
def optimize(pv_p_power, active_load, reactive_load):
    ...
    def SOCP_rule(m, b):
        i = branch_from[b]
        return m.P_ij[b]**2 + m.Q_ij[b]**2 <= m.branch_current[b] * m.Bus_V[i]
    model.SOCP = Constraint(model.BRANCHES, rule=SOCP_rule)
    ...
    if solution_available:
        return model, results, model_build_time
...
decision_times = []  # Pyomo 模型搭建时间 + Gurobi 内部求解时间
feasible = []
objective_values = []    # 目标函数值
network_loss_values = []  # 网损量
curtailment_values = []   # 弃光量
...
for i in range(N_TEST_SAMPLES):
    ...
    # 仅使用 Gurobi 给出的 PV 决策，重新运行标准非凸潮流进行物理可行性检验
    V_pf, l_pf, _, _ = run_powerflow_numpy_single(
        ACTIVE_LOAD_ALL[:, i], REACTIVE_LOAD_ALL[:, i],
        PV_p_actual_values, PV_q_values, PV_bus
    )
    
    feasible.append(check_feasibility(
        V_pf, l_pf, PV_p_actual_values, PV_q_values,
        PV_P_POWER_ALL[i, :], PV_capacity_array
    ))
    decision_times.append(solve_time)
    # 目标函数及其分量
    objective_values.append(value(model.objective))
    network_loss_values.append(np.sum(l_pf * R_vector))
    curtailment_values.append(
        np.sum(PV_P_POWER_ALL[i, :] - PV_p_actual_values) if ENABLE_CURTAILMENT else 0.0
    )
...
results = {
    ...
    'network_loss_values': network_loss_values,
    'curtailment_values': curtailment_values,
    'feasible': feasible,
    'avg_decision_time': np.mean(decision_times),
    'feasibility_rate': np.mean(feasible),
    'avg_objective_value': np.mean(objective_values),
    'avg_network_loss': np.mean(network_loss_values),
    'avg_curtailment': np.mean(curtailment_values)
}

np.savez("Test_result/results_SOC_gurobi.npz", **results)
print(f"\n结果已保存到: Test_result/results_SOC_gurobi.npz")
...
print(f"平均网损量: {np.mean(network_loss_values):.6f}")
print(f"平均弃光量: {np.mean(curtailment_values):.6f}")
