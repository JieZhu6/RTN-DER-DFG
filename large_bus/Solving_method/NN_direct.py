...
def compute_objective_single(l_sq, pv_p, p_available, resistance):
    return np.sum(resistance * l_sq) + np.sum(p_available - pv_p)


def evaluate_periods(x_temporal, y_temporal, pv_bus, pv_capacity, branch_max, resistance):
    """逐时段运行潮流，返回实际调度、可行性和目标值。"""
    n_pv = y_temporal.shape[1] // 2
    n_bus = (x_temporal.shape[1] - n_pv) // 2
    dispatch = np.empty((N_PERIODS, 2 * n_pv))
    feasible = np.empty(N_PERIODS, dtype=bool)
    objectives = np.empty(N_PERIODS)

    for t in range(N_PERIODS):
        p_load = x_temporal[t, :n_bus]
        q_load = x_temporal[t, n_bus : 2 * n_bus]
        p_available = x_temporal[t, 2 * n_bus :]
        pv_p = y_temporal[t, :n_pv] * p_available
        pv_q = y_temporal[t, n_pv:]

        V_sq, l_sq, _, _ = run_powerflow_numpy_single(
            p_load, q_load, pv_p, pv_q, pv_bus
        )
        dispatch[t] = np.concatenate([pv_p, pv_q])
        feasible[t] = check_feasibility(
            V_sq, l_sq, pv_p, pv_q, p_available, pv_capacity
        )
        objectives[t] = compute_objective_single(
            l_sq, pv_p, p_available, resistance
        )

    return dispatch, feasible, objectives
...
def main():
    ...
    model_path = (
        ROOT_DIR
        / "NN_parameter"
        / f"supervised_trained_model_T{N_PERIODS}_{HIDDEN_DIMS}.pth"
    )
    ...
    print("=" * 70)
    print(f"12 时段 NN 直接调度: {n_samples} 个验证样本")
    ...
    result_path = result_dir / "results_V_NN_T12.npz"
    np.savez(
        result_path,
        method="NN_Direct_T12",
