...
12 时段之间没有耦合，因此这里只求解一次单时段鲁棒主问题。生成的同一组
仿射系数由 NN_bisection.py 对每个时段分别调用。
"""
...
# dataset.npy 的每一行仍是单时段运行点；不在此处组合 12 时段。
dataset_path = ROOT_DIR / "Data_generation" / "dataset.npy"
if not dataset_path.exists():
    raise FileNotFoundError(
        f"缺少单时段运行点数据 {dataset_path}，请先运行 training_test_dataset_gen.py"
    )
TEST_DATA = np.load(dataset_path)
N_TEST_SAMPLES = TEST_DATA.shape[0]
expected_single_period_dim = 2 * n_bus + n_pv
if TEST_DATA.ndim != 2 or TEST_DATA.shape[1] != expected_single_period_dim:
    raise ValueError(
        f"单时段内点数据期望形状 (*, {expected_single_period_dim})，"
        f"实际 {TEST_DATA.shape}"
    )
...
baseline_path = ROOT_DIR / 'System_data' / 'baseline_operating_point.npz'
if not baseline_path.exists():
    raise FileNotFoundError(
        f"缺少基准运行点 {baseline_path}，请先运行 generate_baseline_point.py"
    )
...
if V0_mag.shape != (n_bus,) or any(
    array.shape != (n_branches,) for array in (P0_flow, Q0_flow, l0)
):
    raise ValueError(
        "baseline_operating_point.npz 与当前系统维度不一致，请重新运行 "
        "generate_baseline_point.py"
    )
...
if prob.status == cp.OPTIMAL:
    ...
    # 保存结果
    output_path = ROOT_DIR / 'System_data' / 'robust_affine_coefficients.npz'
    np.savez(output_path,
             ...
             n_pv=n_pv,
             n_periods=1)
    print(f"Single-period affine coefficients saved to {output_path}")
else:
